...
void yodificacio_1(char fr_origi[MAX_LL], char taula_origi[MAX_F][MAX_C], char taula_yodif[MAX_F][MAX_C], char fr_yodif[MAX_LL])
{
    ...
    while(taula_origi[i][j]!='.')
    {
        if(taula_origi[i][j]!='\0')
        {
            //Aquí, al ser nivell 1, intercambiarem la fila 3 de la taula original per la 0 de la taula yodificada per a fer aquesta barreja.
            taula_yodif[y][a]=taula_origi[i][j];
            taula_yodif[0][a]=taula_origi[3][j];
            taula_yodif[3][a]=taula_origi[0][j];
            a++;
            j++;
        }
        else
...
void yodificacio_2(char fr_origi[MAX_LL], char taula_origi[MAX_F][MAX_C], char taula_yodif[MAX_F][MAX_C], char fr_yodif[MAX_LL])
{
    ...
    while(taula_origi[i][j]!='.')
    {
        if(taula_origi[i][j]!='\0')
        {
            //Per a donar-li més barreja intercambiarem varies files de la taula original per altres de la taula yodificada.
            taula_yodif[y][a]=taula_origi[i][j];
            taula_yodif[0][a]=taula_origi[1][j];
            taula_yodif[1][a]=taula_origi[0][j];
            taula_yodif[2][a]=taula_origi[3][j];
            taula_yodif[3][a]=taula_origi[2][j];
            a++;
            j++;
        }
        else
...
void printf_veloc()
{
    ...
    printf("Tingues en compte que 0 és considerat com que no vols velocitat_0 i 2 com que en vols molta\n");
}
...
int main()
{
    ...
    else
    {
        ...
        //Finalment, tancarem el fitxer i posarem el missatge conforme s'ha tancat.
        fclose(fit);
