...
extern void laplace_nb(float*, float*, float*, float*, int, int, int);
...
int main(int argc, char* argv[])
{
    ...
    if (ID==0) printf("Inici numprocessos=%d (files=%d)NON-BLOCKING\n", numprocessos, files);

    double t0 = MPI_Wtime();
    laplace_nb(A, B, daprev, danext, N, LD, Niter);
    double t1 = MPI_Wtime();
