...
void * mou_fantasma(void * index)
{
  ...
  do {
    ...
    for (k=-1; k<=1; k++)		/* provar direccio actual i dir. veines */
    {
      ...
      seg.f = fant[i].f + df[vk]; /* calcular posicio en la nova dir.*/
      seg.c = fant[i].c + dc[vk];
    ...
    { if (nd == 1)			/* si nomes pot en una direccio */
      ...
      seg.f = fant[i].f + df[fant[i].d];  /* calcular seguent posicio final */
      seg.c = fant[i].c + dc[fant[i].d];
      seg.a = win_quincar(seg.f,seg.c);	/* calcular caracter seguent posicio */
...
int main(int n_args, const char *ll_args[])
{
  ...
  if (rc == 0)		/* si aconsegueix accedir a l'entorn CURSES */
  {
    ...
    pthread_create(&tid[1], NULL, mou_menjacocos, (void *) (intptr_t) reta);
    nfant=nfant+2;
    while(i<nfant)
    {
