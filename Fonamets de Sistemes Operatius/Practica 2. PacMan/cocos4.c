...
void carrega_parametres(const char *nom_fit)
{
  ...
  while (!feof(fit))
  {
    if(nfant<10)
    {
        fscanf(fit,"%d %d %d %f\n",&fant[nfant].f,&fant[nfant].c,&fant[nfant].d,&fant[nfant].r);
        if ((fant[nfant].f < 1) || (fant[nfant].f > n_fil1-3) || (fant[nfant].c < 1) || (fant[nfant].c > n_col-2) || (fant[nfant].d < 0) || (fant[nfant].d > 3))
        {
	        fprintf(stderr,"Error: parametres fantasma 1 incorrectes:\n");
	        fprintf(stderr,"\t1 =< fant.f (%d) =< n_fil1-3 (%d)\n",fant[nfant].f,(n_fil1-3));
	        fprintf(stderr,"\t1 =< fant.c (%d) =< n_col-2 (%d)\n",fant[nfant].c,(n_col-2));
	        fprintf(stderr,"\t0 =< fant.d (%d) =< 3\n",fant[nfant].d);
	        fclose(fit);
	        exit(5);
        }
    }
...
int inicialitza_joc(void)
{
  ...
  if (r == 0)
  {
    ...
    else
    {
        ...
        while(i<nfant)
        {
            fant[i].a = win_quincar(fant[i].f,fant[i].c);
            if (fant[i].a == c_req) r = -7;	/* error: fantasma sobre pared */
            i++;
        }
        if(r==0)
        {
	...
	for (i=0; i<n_fil1-1; i++)
	  for (j=0; j<n_col; j++)
        ...
        {
	        numfant=i+49;
	        win_escricar(fant[i].f,fant[i].c,numfant,NO_INV);
        }
...
int main(int n_args, const char *ll_args[])
{
  ...
  ret2=0; ret=0;
  int reta=0;
  char m[20], f[20], c[20], d[20], r[20], a[20], fil[20], col[20], ind[20], ret_ard[20];
  ...
  if (rc == 0)		/* si aconsegueix accedir a l'entorn CURSES */
  {
    for (int i = 0; i < nfant; i++)
    {
      ...
      //fprintf(stderr, "tpid: %d\n", tpid[n]);
      if (tpid[n] == (pid_t) 0)		/* branca del fill */
      {
        sprintf(m,"%i",id_mem);
        //fprintf(stderr,"id_mem: %s\n", m);
        sprintf(fil,"%i",n_fil1);
        //fprintf(stderr,"fil: %s\n", fil);
        sprintf(col,"%i",n_col);
        //fprintf(stderr,"col: %s\n", col);
        sprintf(f,"%i",fant[i].f);
        //fprintf(stderr,"f: %s\n", f);
        sprintf(c,"%i",fant[i].c);
        //fprintf(stderr,"c: %s\n", c);
        sprintf(d,"%i",fant[i].d);
        //fprintf(stderr,"d: %s\n", d);
        sprintf(r,"%f",fant[i].r);
        //fprintf(stderr,"fant.r: %s\n", r);
        sprintf(a,"%i",fant[i].a);
        //fprintf(stderr,"fant.a: %s\n", a);
        sprintf(ind,"%i",i);
        //fprintf(stderr,"ind: %s\n", ind);
        sprintf(ret_ard,"%i",retard);
        execlp("./fantasma3", m, fil, col, f, c, d, r, a, ind, ret_ard, (char *)0);
	      fprintf(stderr,"error: no puc executar el process fill \'fantasma3.c\'\n");
	      exit(0);
      }
      else if (tpid[n] > 0) n++;		/* branca del pare */
    }
    mou_menjacocos((void *) (intptr_t) reta);

    for (n = 0; n < nfant; n++)
    {
      waitpid(tpid[n],NULL,0);
    }
    ...
    {
      temps=temps_joc/60;
      temps_joc=temps_joc%60;
    }
    printf("TEMPS DE JOC: %d:%d\n", temps, temps_joc);
    
    if (ret == -1) printf("S'ha aturat el joc amb tecla RETURN!\n");
    else
    {
        if (ret == 1) printf("Ha guanyat l'usuari!\n");
